...
import { sendMessage, importConversation, listConversations } from "@/app/knowledge/actions";
...
export default function KnowledgeBasePage() {
  ...
  async function handleSend(text?: string) {
    ...
    const result = await sendMessage(activeConvId, msg);
    if (!activeConvId) {
  ...
  async function handleImport() {
    ...
    await importConversation(importText, importSource);
    setShowImport(false);
    setImportText("");
    listConversations(undefined, 20).then(setConversations);
  }
    ...
    <div className="flex h-[calc(100vh-0px)] overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 shrink-0 bg-[#0d0d0d] border-r border-white/10 flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => setShowImport(true)} className="text-[10px] text-zinc-500 hover:text-zinc-300 transition-colors">
              + Import
            </button>
          </div>
          <div className="space-y-1">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="md:hidden flex items-center justify-between px-4 py-2 border-b border-white/10 shrink-0">
          <span className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Ask BUILT</span>
          <button onClick={newConversation}
          ...
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] px-4 py-3 rounded-2xl text-sm whitespace-pre-wrap ${
                m.role === 'user' ? 'bg-built-red text-white rounded-br-sm' : 'bg-[#1a1a1a] text-zinc-200 border border-white/10 rounded-bl-sm'
              }`}>{m.content}</div>
            </div>
          ))}
          {loading && (
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex gap-3">
            ...
            <button onClick={() => handleSend()} disabled={loading || !input.trim()}
              className="bg-built-red hover:bg-built-red/90 disabled:opacity-40 text-white px-5 py-3 rounded-xl text-sm font-semibold transition-all">→</button>
          </div>
        </div>
      </div>
      ...
      {showImport && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#111111] border border-white/10 rounded-2xl p-6 w-full max-w-lg">
            ...
            <select value={importSource} onChange={e => setImportSource(e.target.value as "claude_import" | "gemini_import")}
            ...
            />
            <div className="flex gap-3">
              <button onClick={() => setShowImport(false)} className="flex-1 py-2.5 rounded-lg border border-white/10 text-sm text-zinc-400 hover:bg-white/5">Anulează</button>
              <button onClick={handleImport} className="flex-1 py-2.5 rounded-lg bg-built-red text-white text-sm font-semibold hover:bg-built-red/90">Importă</button>
            </div>
          </div>
        </div>
      )}
