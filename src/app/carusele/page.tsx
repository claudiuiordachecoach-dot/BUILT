...
import { SplitCarouselTab } from "@/components/carusele/SplitCarouselTab";

type Tab = "generator" | "agent" | "split";
  ...
  return (
    <div className="p-8 max-w-6xl">
      ...
      <div className="flex gap-1 mb-8 border-b border-built-gray-2">
        {(["generator", "agent", "split"] as Tab[]).map((tab) => (
          <button key={tab} type="button" onClick={() => setActiveTab(tab)}
            ...
            {tab === "generator" ? "Generator" : tab === "agent" ? "Agent ✦" : "Split (Before/After)"}
          </button>
        ))}
      </div>
      ...
      {activeTab === "split" && <SplitCarouselTab />}
    </div>
  );
}
